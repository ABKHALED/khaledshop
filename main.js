...
// get and set elements in the featured section
let featHolder = document.querySelector(".feat-holder");
let addCartBtn;
function get3Elements() {
  featHolder.innerHTML = `<div class='lodser'></div>`;
  fetch("https://course-api.com/javascript-store-products")
    ...
    .then((res) => {
      let d = res.filter((ele) => {
        return ele.fields.featured === true;
      });
      return d;
    })
    .then((res) => {
                     ...
                     <div class="left-side">
                      <img src="${ele.fields.image[0].url}" alt="">
                           ...
                           <div class="right-side">
                               <i  class="fa-solid fa-arrow-up"></i>
                               <span id="${ele.id}" class="pice">0</span>
                               <i id="${ele.id}" class="fa-solid fa-arrow-down"></i>
                           </div>`;
              })
              .join("");
            sliderHolder.innerHTML += p;
